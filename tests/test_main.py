...
def produto_id(test_client):
    ...
    response = test_client.post("/produto", json=produto_data)
    data = response.json()
    return data["id"]
...
def test_obter_produto(test_client, produto_id):
    """
    Testa a obtenção de um produto específico através da rota GET '/produto/{produto_id}'.
    Verifica se o produto obtido corresponde ao produto criado pela fixture 'produto_id'.
    """
    response = test_client.get(f"/produto/{produto_id}")
    assert response.status_code == 200
    data = response.json()
...
def test_atualizar_produto(test_client, produto_id):
    """
    Testa a atualização de um produto existente pela rota PUT '/produto/{produto_id}'.
    ...
    response = test_client.put(f"/produto/{produto_id}", json=novo_dado)
    assert response.status_code == 200
    data = response.json()
...
def test_remover_produto(test_client, produto_id):
    """
    Testa a remoção de um produto pela rota DELETE '/produto/{produto_id}'.
    Verifica se o produto é removido com sucesso e se o mesmo não é mais encontrado após a remoção.
    """
    response = test_client.delete(f"/produto/{produto_id}")
    assert response.status_code == 200
    response = test_client.get(f"/produto/{produto_id}")
...
def test_modelo_produto_valido():
    produto = ProdutosSchema(titulo="Teste", descricao="Descrição Teste", preco=10.0)
